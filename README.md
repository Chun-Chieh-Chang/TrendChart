...
- **過濾引擎**：多維度動態篩選，**支援關鍵字匹配搜尋**，同步更新計算與圖表。
...
| **參數設定** | 智慧規格 (Spec) | **自動搜尋篩選結果中首筆有效數字**作為 Target/USL/LSL。 |
| | 軸向記憶 | 紀錄 X/Y 軸欄位，換表不跑掉。 |
| **品質指標** | 能力分析 | 自動計算關鍵品質指標 (Cpk, Ppk) 支援 4 位精度。 |
| | 管制界限 | 基於組內變異計算 ±3σ 管制界限。 |
| **數據篩選** | **匹配搜尋** | 篩選下拉選單支援關鍵字搜尋，快速鎖定特定批次或料號。 |
| | 重置功能 | 一鍵清除所有篩選條件。 |
| **效能優化** | **GPU 加速** | 大量數據點自動啟用 WebGL 渲染，縮放不卡頓。 |
| | **預覽控制** | 可手動關閉數據預覽表格，並採用懶加載技術節省記憶體。 |
