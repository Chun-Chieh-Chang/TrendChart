...
- **極簡視覺系統**：採用**紙白 (`#F7F7F7`) × 墨黑 (`#111`) × 髮絲線分隔 × 底線輸入框 × 細線圓點開關**，介面純黑白灰、無卡片無陰影；僅數據判讀（規格外點、規格 / 管制 / 目標線、Cpk 等級）保留低彩度語意色（PNG 匯出自動白底）。
...
| | 異常點高亮 | 超出規格自動高亮為低彩度警示紅點 (`#b4443c`)。 |
| | **雙 X 軸色階** | 主軸與副軸文字交替採用高對比深色階（墨黑/中灰 vs 深灰/淺灰）。 |
| | **極簡主義風格** | 髮絲線分隔、大寫寬字距標籤、底線輸入框、細線圓點開關、底線分頁、細線圓形圖示按鈕、左側豎線卡片。 |
...
- **`css/`**：視覺風格定義（Minimalism 極簡主義設計系統）。
...
- **視覺風格**：全站套用 **極簡主義 (Minimalism)**：`#F7F7F7` 紙白底、`#111` 墨黑文字、`#E4E4E4` 1px 髮絲線分隔，不使用卡片、陰影與品牌色；大寫寬字距 (0.16em) 小標籤、細字重 (300) 數值。語意色僅用於數據判讀且降低彩度：紅 `#b4443c`、琥珀 `#a8741a`、綠 `#4f7a5f`、藍灰 `#4a6785`。尊重 `prefers-reduced-motion`。
