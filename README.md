...
4. **篩選 (Refine)**：利用側邊欄動態搜尋並選取特定條件，上方指標即時同步更新。
5. **分析 (Analysis)**：觀察自動計算的 Ca/Cp/Cpk/Ppk、管制界限 (UCL/LCL) 與常態分佈曲線；可透過「標籤位置」切換標籤至左或右，避免遮擋數據點。
6. **導出 (Export)**：一鍵下載高品質 PNG 圖表（趨勢圖與常態分佈圖各自獨立匯出）。
