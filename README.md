...
本工具具備靈活的數據對比與整合能力，專為製程數據分析設計：
- **多維度欄位對照**：支援在單一圖表中同時選取多個 Y 軸欄位，快速進行不同數據項目的趨勢比較與相關性觀察。
- **跨工作表整合分析**：透過強大的數據拼接技術，即使數據分佈在不同工作表分頁，只要其欄位結構一致，即可透過「按住 Ctrl 多選」將其整構成單一數據源，實現跨批次或跨時段的長期趨勢追蹤。
