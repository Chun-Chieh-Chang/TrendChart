...
const ChartRenderer = (() => {
    ...
    const renderTrendChart = (data, xColumn, yColumns, specs = {}, stats = null, targetId = 'plotly-trend', sheetName = '', xColumn2 = '', isXDate = false, isX2Date = false) => {
        ...
        if (specs.showTarget !== false) {
            addLimitLine(specs.target, 'Target', '#3fa96b', '40px 10px 10px 10px', 2, 'target');
        }

        if (specs.showSpec !== false) {
            addLimitLine(specs.usl, 'USL', '#e0625b', 'dash', 1.5, 'usl');
            addLimitLine(specs.lsl, 'LSL', '#e0625b', 'dash', 1.5, 'lsl');
        }
    ...
    const renderNormalDistChart = (data, columns, specs = {}, stats = null, targetId = 'plotly-dist', sheetName = '') => {
        ...
        if (specs.showTarget !== false) {
            addLimit(specs.target, 'Target', '#3fa96b', '40px 10px 10px 10px', 2, 'target');
        }

        if (specs.showSpec !== false) {
            addRange(specs.lsl, specs.usl, 'rgba(224, 98, 91, 0.05)');
            addLimit(specs.usl, 'USL', '#e0625b', 'dash', 1.5, 'usl');
            addLimit(specs.lsl, 'LSL', '#e0625b', 'dash', 1.5, 'lsl');
        }
